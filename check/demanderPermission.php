<?php
	...
	$username_destinataire = DestroyHTML($_GET['username']);
	$id_exp = intval($_SESSION['userid']);

	$username_destinataire=str_replace("@","",$username_destinataire);
	$profile = $bdd->query("SELECT * FROM users WHERE username= :username",array("username"=>$username_destinataire));
	$id_destinataire=$profile[0]['id'];
	$moi = $bdd->query("SELECT * FROM users WHERE id= :userid",array("userid"=>$_SESSION['userid']));
	...
	$t["erreur"] = "no";
	header("Location: ../profile/".$profile[0]['username']."?message=requestOk");
